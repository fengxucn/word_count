...
public class Utils {
    ...
    public static int map(String path) {
        // Number of executors
        int n = getExecutorNumber();
        try {
            ...
            //calculate how many executor needed
            int m = (int) (size / MAX_SIZE_EACH_TASK()) + 1;
            n = Math.min(m, n);

            for (int i = 0; i < n; i++) {
                Executor object = new Executor(i, path, n, size);
                object.start();
            }
